...
public class sock_layer<T extends LivingEntity> extends BipedEntityModel<T> {
	...
	public static TexturedModelData getModelData(float dilation) {
		...
		modelPartData.addChild(EntityModelPartNames.LEFT_LEG, ModelPartBuilder.create().uv(0, 0).mirrored().cuboid(-2.0f, 0.0f, -2.0f, 4.0f, 12f, 4.0f, new Dilation(dilation)), ModelTransform.pivot(1.9f, 12.0f, 0.0f));
